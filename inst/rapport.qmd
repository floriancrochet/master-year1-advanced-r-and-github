...
params:
  ...
  data.frame_name: df_gers_loire_atlantique # Valeur par défaut
...
# remotes::install_github("floriancrochet/masters-year1-advanced-r-and-github", quiet = TRUE)
...
```{r}
if (params$data.frame_name == "df_gers_loire_atlantique") {
  # Jeu de données du package
  data <- df_gers_loire_atlantique
} else {
  # Jeu de données en paramètre
  data <- readRDS(normalizePath(params$data.frame_path, winslash = "/", mustWork = TRUE))
}
```
